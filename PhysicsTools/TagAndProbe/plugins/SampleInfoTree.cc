...
tnp::SampleInfoTree::SampleInfoTree(const edm::ParameterSet& iConfig) : 
  ...
  produces<edm::MergeableCounter, edm::InLumi> ("totalGenWeight"); 
  produces<edm::MergeableCounter, edm::InLumi> ("totalEvent");
}
...
void tnp::SampleInfoTree::endLuminosityBlockProduce(edm::LuminosityBlock & theLuminosityBlock, const edm::EventSetup & theSetup) {
  auto numWeightsPtr = std::make_unique<edm::MergeableCounter>();
  numWeightsPtr->value = sumWeight_;
  theLuminosityBlock.put(std::move(numWeightsPtr), "totalGenWeight");
