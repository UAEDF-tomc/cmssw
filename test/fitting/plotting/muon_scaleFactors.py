...
def diagnosticErrorPlot( effgr, ierror, errName, nameout ):
    c2D_Err = rt.TCanvas('canScaleFactor_%s' % errName ,'canScaleFactor: %s' % errName,1000,600)    
    c2D_Err.Divide(2,1)
    ...
    h2_sfErrorAbs.SetTitle('absolute SF syst: %s ' % errName)
    h2_sfErrorRel.SetTitle('relative SF syst: %s ' % errName)
    c2D_Err.cd(1)
...
nameOutBase = "./"+ outputDirectory + "/" + filein.split('/')[-1].replace('TTW','ttW').replace('TTZ','ttZ') 
...
for line in fileWithEff:
    ...
    if len(numbers) > 0:
      etaKey = (numbers[2], numbers[3])
      ptKey  = (numbers[0], numbers[1])
      myeff = efficiency(ptKey,etaKey,*numbers[4:])
...
canvas.SaveAs(os.path.join(nameOutBase.split('eff_all_')[-1].split('.txt')[0].split('_eta')[0] + ".pdf"))

for isyst, name in enumerate(efficiency.getSystematicNames()):
    diagnosticErrorPlot( effGraph, isyst, name, pdfout )
